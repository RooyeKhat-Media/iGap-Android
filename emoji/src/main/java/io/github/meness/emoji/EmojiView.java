...
import io.github.meness.emoji.emoji.Cars;
import io.github.meness.emoji.emoji.Electronics;
import io.github.meness.emoji.emoji.Emoji;
import io.github.meness.emoji.emoji.Food;
import io.github.meness.emoji.emoji.Nature;
import io.github.meness.emoji.emoji.People;
import io.github.meness.emoji.emoji.Sport;
import io.github.meness.emoji.emoji.Symbols;
...
import io.github.meness.emoji.listeners.RepeatListener;

@SuppressLint("ViewConstructor")
final class EmojiView extends LinearLayout
    ...
    EmojiView(final Context context, final OnEmojiClickedListener onEmojiClickedListener,
        ...
        findViewById(R.id.emojis_backspace).setOnTouchListener(
    ...
    @NonNull
    private List<? extends View> getViews(final Context context,
                                          @Nullable final OnEmojiClickedListener onEmojiClickedListener,
                                          @NonNull final RecentEmoji recentEmoji) {
        recentGridView = new RecentEmojiGridView(context).init(onEmojiClickedListener, recentEmoji);
        final EmojiGridView peopleGridView =
                createGridView(context, People.DATA, onEmojiClickedListener);
        final EmojiGridView natureGridView =
                createGridView(context, Nature.DATA, onEmojiClickedListener);
        ...
        final EmojiGridView electronicsGridView =
                createGridView(context, Electronics.DATA, onEmojiClickedListener);
        final EmojiGridView symbolsGridView =
                createGridView(context, Symbols.DATA, onEmojiClickedListener);
        return Arrays.asList(recentGridView, peopleGridView, natureGridView, foodGridView,
                sportGridView, carsGridView, electronicsGridView, symbolsGridView);
    }

    private EmojiGridView createGridView(@NonNull final Context context, final Emoji[] emojis,
                                         @Nullable final OnEmojiClickedListener onEmojiClickedListener) {
